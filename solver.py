...
class DfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        if puzzle.is_solved() and (seen is None or str(puzzle) not in seen):
            ...
            return []
        elif seen is None or str(puzzle) not in seen:
            if seen is None:
                seen = {str(puzzle)}
            else:
                seen.add(str(puzzle))
...
class BfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        while not q.is_empty():
            lst = q.dequeue()
            if lst[-1].is_solved() and \
                    (seen is None or str(lst[-1]) not in seen):
                return lst
            elif str(lst[-1]) not in seen:
                seen.add(str(lst[-1]))
                for item in lst[-1].extensions():
                    temp = lst + [item]
                    q.enqueue(temp)
        return []
