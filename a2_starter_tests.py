...
def test_sudoku_fail_fast() -> None:
    s = SudokuPuzzle(4, [["B", "D", "A", "C"],
                         ["A", "C", "B", "D"],
                         ["C", "B", "D", "A"],
                         ["D", "A", "C", "B"]],
...
def test_sudoku_fail_fast1() -> None:
    s = SudokuPuzzle(4, [["B", "D", "A", "C"],
                         ["C", "A", "B", "D"],
                         ["A", "B", "C", "D"],
                         ["A", "A", "A", "A"]],
...
def test_sudoku_fail_fast2() -> None:
    s = SudokuPuzzle(4, [[" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", " "]],
...
def test_sudoku_fail_fast3() -> None:
    s = SudokuPuzzle(4, [["A", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", " "]],
...
def test_sudoku_fail_fast4() -> None:
    s = SudokuPuzzle(4, [["A", "A", "D", "C"],
                         ["B", "B", "C", "B"],
                         ["C", "C", "B", "A"],
                         ["D", "D", "A", " "]],
                     {"A", "B", "C", "D"})
...
def test_sudoku_fail_fast5() -> None:
    s = SudokuPuzzle(4, [[" ", "A", "D", "C"],
                         ["B", "B", "C", "B"],
                         ["C", "C", "B", "A"],
                         ["D", "D", "A", "B"]],
                     {"A", "B", "C", "D"})
...
def test_sudoku_fail_fast6() -> None:
    s = SudokuPuzzle(4, [["A", "A", "D", "C"],
                         ["B", " ", " ", "B"],
                         ["C", "C", "B", "A"],
                         ["D", "D", "A", " "]],
...
def test_sudoku_fail_fast7() -> None:
    s = SudokuPuzzle(4, [["A", "A", "D", "C"],
                         ["B", "B", "C", "B"],
                         [" ", "C", "B", "A"],
                         ["D", "D", " ", "C"]],
...
def test_sudoku_fail_fast8() -> None:
    s = SudokuPuzzle(4, [[" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", " ", " ", "A"]],
...
def test_sudoku_fail_fast9() -> None:
    s = SudokuPuzzle(4, [[" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", "A", "B", " "],
                         [" ", " ", " ", " "]],
...
def test_sudoku_fail_fast10() -> None:
    s = SudokuPuzzle(4,[["B", " ", "B", " "],
                        ["B", " ", "D", "C"],
                        [" ", " ", "A", " "],
                        [" ", " ", "C", " "]], {"A", "B", "C", "D"})
    assert s.fail_fast() is False
...
def test_has_unique_solution_doctest1() -> None:
    s = SudokuPuzzle(4, [[" ", " ", " ", " "],
                         [" ", " ", " ", " "],
                         [" ", "A", "B", " "],
                         [" ", " ", " ", " "]],
...
def test_word_ladder_eq_1() -> None:
    wl1 = WordLadderPuzzle("me", "my", {"me", "my", "ma"})
    wl2 = WordLadderPuzzle("me", "my", {"me", "my", "mu", "mo"})
    assert wl1.__eq__(wl2) is False


def test_word_ladder_eq_2() -> None:
    wl1 = WordLadderPuzzle("me", "my", {"me", "my", "ma"})
    wl2 = WordLadderPuzzle("me", "my", {"me", "my"})
    assert wl1.__eq__(wl2) is False


def test_word_ladder_eq_3() -> None:
    wl1 = WordLadderPuzzle("me", "my", {"me", "my", "ma"})
    wl2 = WordLadderPuzzle("me", "mo", {"me", "my", "ma"})
    assert wl1.__eq__(wl2) is False


def test_word_ladder_eq_4() -> None:
    wl1 = WordLadderPuzzle("me", "my", {"me", "my", "ma"})
    wl2 = WordLadderPuzzle("mo", "my", {"me", "my", "ma"})
    assert wl1.__eq__(wl2) is False
...
def test_word_ladder_str_1() -> None:
    wl1 = WordLadderPuzzle("my", "me", {"me", "my", "ma"})
    wl2 = WordLadderPuzzle("mo", "my", {"me", "my", "mu"})
    assert str(wl1) == 'my -> me'
    assert str(wl2) == 'mo -> my'
...
def test_word_ladder_get_difficulty1() -> None:
    wl2 = WordLadderPuzzle("tear", "cost", {"cost", "tear", "teat",
                                            "test", "tost"})
    assert wl2.get_difficulty() == MEDIUM


def test_word_ladder_get_difficulty2() -> None:
    wl2 = WordLadderPuzzle("feds", "cost", {"cost", "feds", "reds", "rods",
                                            "cods", "cols", "colt"})
    assert wl2.get_difficulty() == HARD
...
def test_tree_eval() -> None:
    new = ExprTree('+',
                   [ExprTree(3, []), ExprTree('*', [ExprTree('b', []),
                                                    ExprTree('+', [ExprTree(2, []), ExprTree('d', [])])]),
                    ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
                                                      ExprTree('c', [])])])
    lookup = {'a':7}
    new.populate_lookup(lookup)
    assert new.eval(lookup) == 8

    lookup['a'] = 9
    assert new.eval(lookup) == 17
...
def test_populate_lookup() -> None:
    new = ExprTree('+', [ExprTree(3, []), ExprTree('*', [ExprTree('x', []),
                                                         ExprTree('y', [])]),
                         ExprTree('x', [])])
    look_up = {}
    new.populate_lookup(look_up)
    assert len(look_up) == 2
    assert look_up['x'] == 0
    assert look_up['y'] == 0


def test_populate_lookup1() -> None:
    new = ExprTree('+',
                   [ExprTree(3, []), ExprTree('*', [ExprTree('b', []),
                    ExprTree('+', [ExprTree(2, []), ExprTree('d', [])])]),
                    ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
...
def test_expression_tree_construct_from_list_1() -> None:
    example2 = [['+'], [3, '*', 'a', '+'], ['b', '+'], [5, 'c'], [2, 'd']]
    get = construct_from_list(example2)
    assert str(get) == '(3 + (b * (2 + d)) + a + (5 + c))'


def test_construct() -> None:
    start = [['*'], ['a', '*'], ['a', '*'], ['a', 'a']]
    res = construct_from_list(start)
    assert str(res) == '(a * (a * (a * a)))'


def test_construct1() -> None:
    start = [['+'], [3, '*', 'a', '+'], ['*', '*'], [5, 'c'],['1','1'],['1','1']]
    res = construct_from_list(start)
    assert str(res) == '(3 + ((1 * 1) * (1 * 1)) + a + (5 + c))'
...
def test_expression_tree_substitute1() -> None:
    new = ExprTree('+',
                   [ExprTree(3, []), ExprTree('*', [ExprTree('b', []),
                    ...
                    ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
                                                      ExprTree('c', [])])])
    look_up = {'a': 1, 'b': 1, 'c': 1, 'd': 1}
    new.substitute(look_up)
    assert str(new) == '(3 + (1 * (2 + 1)) + 1 + (5 + 1))'
...
def test_expression_tree_eq_() -> None:
    t1 = ExprTree('*', [ExprTree(5, []), ExprTree(2, [])])
    assert t1.__eq__(ExprTree('*', [ExprTree(5, []), ExprTree(3, [])])) is False


def test_expression_tree_eq_1() -> None:
    t1 = ExprTree('*', [ExprTree(5, []), ExprTree(2, [])])
    assert t1.__eq__(ExprTree('*', [ExprTree(5, [])])) is False


def test_expression_tree_eq_2() -> None:
    t1 = ExprTree('+', [ExprTree(5, []), ExprTree(2, [])])
    assert t1.__eq__(ExprTree('*', [ExprTree(5, []), ExprTree(2, [])])) is False


def test_expression_tree_eq_3() -> None:
    t1 = ExprTree('*',
                  [ExprTree(5, []), ExprTree('*', [ExprTree('b', []),
                   ExprTree('+', [ExprTree(2, []), ExprTree('d', [])])]),
                   ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
                                                     ExprTree('c', [])])])
    assert t1.__eq__(ExprTree('*', [ExprTree(5, []), ExprTree(2, [])])) is False
...
def test_expression_tree_puzzle_is_solved1() -> None:
    t1 = ExprTree('+',
                  [ExprTree(3, []), ExprTree('*', [ExprTree('b', []),
                   ExprTree('+', [ExprTree(2, []), ExprTree('d', [])])]),
                   ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
                                                     ExprTree('c', [])])])
    puz = ExpressionTreePuzzle(t1, 13)
    assert puz.is_solved() is False
    puz.variables['a'] = 13
    assert puz.is_solved() is False
    puz.variables['a'] = 1
...
def test_expression_tree_puzzle_str() -> None:
    t1 = ExprTree('+',
                  [ExprTree(3, []), ExprTree('*', [ExprTree('b', []),
                   ExprTree('+', [ExprTree(2, []), ExprTree('d', [])])]),
                   ExprTree('a', []), ExprTree('+', [ExprTree(5, []),
