...
class ExpressionTreePuzzle(Puzzle):
    ...
    def extensions(self) -> List[ExpressionTreePuzzle]:
        ...
        for item in lst:
            for i in range(1, 10):
                ...
                new_dict[item] = i
                new_tree.substitute({item: i})
                new_puzzle = ExpressionTreePuzzle(new_tree, self.target)
                new_puzzle.variables = new_dict
    ...
    def fail_fast(self) -> bool:
        ...
        >>> a = ExpressionTreePuzzle(ExprTree('+', [ExprTree(\
        '*', [ExprTree(5, []), ExprTree('a', [])]), ExprTree(3, [])]), 8)
        >>> a.fail_fast()
        """
        if self.is_solved():
            return False
        if self._tree.is_empty() or \
                self.target <= 0 or \
                self._tree.eval(self.variables) > self.target or \
                not self.extensions():
            return True
        lst = [x.fail_fast() for x in self.extensions()]
        return all(lst)
